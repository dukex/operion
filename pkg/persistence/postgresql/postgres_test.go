...
func setupTestDB(t *testing.T) *PostgreSQLPersistence {
	...
	persistence, err := NewPostgreSQLPersistence(logger, databaseURL)
	require.NoError(t, err)

	// Clean up any existing test data
	db := persistence.(*PostgreSQLPersistence).db
	_, err = db.Exec("DELETE FROM workflows WHERE id LIKE 'test-%'")
	require.NoError(t, err)

	return persistence.(*PostgreSQLPersistence)
}

func TestPostgreSQLPersistence_Migrations(t *testing.T) {
	...
	_, _ = db.Exec("DROP TABLE IF EXISTS workflows")
	...
	var exists bool
	err = db.QueryRow("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'workflows')").Scan(&exists)
	require.NoError(t, err)
...
func TestPostgreSQLPersistence_HealthCheck(t *testing.T) {
	p := setupTestDB(t)
	defer p.Close()
...
func TestPostgreSQLPersistence_SaveAndRetrieveWorkflow(t *testing.T) {
	p := setupTestDB(t)
	defer p.Close()
	...
	assert.Equal(t, len(workflow.WorkflowTriggers), len(retrieved.WorkflowTriggers))
	...
	notFound, err := p.WorkflowByID("non-existent")
	require.NoError(t, err)
	assert.Nil(t, notFound)
}

func TestPostgreSQLPersistence_UpdateWorkflow(t *testing.T) {
	p := setupTestDB(t)
	defer p.Close()
	...
	// Verify update
	retrieved, err := p.WorkflowByID("test-workflow-2")
	require.NoError(t, err)
	require.NotNil(t, retrieved)
...
func TestPostgreSQLPersistence_ListWorkflows(t *testing.T) {
	p := setupTestDB(t)
	defer p.Close()
...
func TestPostgreSQLPersistence_DeleteWorkflow(t *testing.T) {
	p := setupTestDB(t)
	defer p.Close()
	...
	// Verify it exists
	retrieved, err := p.WorkflowByID("test-workflow-5")
	require.NoError(t, err)
	require.NotNil(t, retrieved)

	// Delete workflow
	err = p.DeleteWorkflow("test-workflow-5")
	require.NoError(t, err)

	// Verify it's gone (soft delete)
	deleted, err := p.WorkflowByID("test-workflow-5")
	require.NoError(t, err)
	assert.Nil(t, deleted)

	// Delete non-existent workflow (should not error)
	err = p.DeleteWorkflow("non-existent")
	assert.NoError(t, err)
}

func TestPostgreSQLPersistence_ComplexWorkflow(t *testing.T) {
	p := setupTestDB(t)
	defer p.Close()
	...
	// Retrieve and verify
	retrieved, err := p.WorkflowByID("test-complex-workflow")
	require.NoError(t, err)
	require.NotNil(t, retrieved)
