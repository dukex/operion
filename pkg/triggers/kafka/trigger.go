...
var (
	ErrTopicRequired   = errors.New("kafka trigger topic is required")
	ErrBrokersRequired = errors.New("kafka trigger brokers are required")
)
	...
	go func() {
		...
		for {
			...
			case <-childCtx.Done():
				...
				return
			default:
				err := consumer.Consume(childCtx, []string{t.Topic}, handler)
				if err != nil {
