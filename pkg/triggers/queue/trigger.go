...
var (
	ErrQueueNameRequired        = errors.New("queue trigger queue name is required")
...
func (t *QueueTrigger) Validate() error {
	...
	if t.Provider != "redis" {
		return fmt.Errorf("%w: %s (only 'redis' is supported)", ErrUnsupportedQueueProvider, t.Provider)
	}
...
func (t *QueueTrigger) initializeClient(ctx context.Context) error {
	...
	err := t.client.Ping(timeoutCtx).Err()
	if err != nil {
