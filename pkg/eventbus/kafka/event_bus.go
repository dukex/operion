...
func NewEventBus(ctx context.Context, logger *slog.Logger) (eventbus.EventBus, error) {
	...
	tracer, err := otelhelper.NewTracer(ctx, "operion-kafka-event-bus")
	if err != nil {
		logger.ErrorContext(ctx, "Failed to create tracer", "error", err)
...
func (k *kafkaEventBus) Subscribe(ctx context.Context) error {
	...
	go consumeEvents(ctx, k.logger, k.reader, k.tracer, k.handlers)
...
func (k *kafkaEventBus) GenerateID(ctx context.Context) string {
	...
	k.logger.InfoContext(ctx, "Generated new ID", "id", id.String())

	return id.String()
}

func (k *kafkaEventBus) Handle(ctx context.Context, eventType events.EventType, handler eventbus.EventHandler) error {
	k.logger.InfoContext(ctx, "Handling message", "message", string(eventType))
